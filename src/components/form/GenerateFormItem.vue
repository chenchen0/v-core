<template>
    <el-form-item :widget-type="widget.type" :label="widget.type == 'button' ? '' : widget.name" :prop="widget.model">
        <template v-if="widget.type == 'label'">
            <div class="fm-generate-label-ctn">{{ dataModel == null ? devModel : dataModel }}</div>
        </template>
        <template v-if="widget.type == 'button'">
            <el-button
                ...
                @click="remote && remote[widget.options.remoteFunc] && remote[widget.options.remoteFunc]()"
            >
...
export default {
    ...
    data() {
        return {
            ...
            devModel: process.env.NODE_ENV == "development" ? "测试内容文本" : null
        };
    },
    created() {
